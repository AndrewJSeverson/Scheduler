    ...
    public class RoundRobin : Scheduler
    {
        ...
        public override SchedulerResult Run(List<ProcessItem> processes)
        {
            ...
            while (processes.Count > 0)
            {
                    ...
                    if (cur % 2 == 0 && processes[i].ArrivalTime <= currentTime && cpuTime <= currentTime)
                    {
                        availableCPUs.Add(
                            new EamonProcess
                            {
                                ...
                                ProcessIndex = i,
                                BurstRemaining = processes[i].BurstArray[cur]
                            }
                        );
                    }
                ...
                {
                    EamonProcess first = availableCPUs.First();
                    cpuProcesses.Add(first);

                    // how much time we spent working on a process.
                    // This will either be the value of the quantum, or the value of the process duration < quantum.
                    int timeSpent;

                    // We have a time quantum to worry about here. 

                    // If what we have to do will fit within a quantum, just do it and move on.
                    if (first.BurstRemaining < quantum)
                    {
                        cpuTime = timeSpent = first.BurstRemaining;
                        first.BurstRemaining = 0;
                        availableCPUs.RemoveAt(0);
                    }
                    else
                    {
                        cpuTime = timeSpent = quantum;
                        first.BurstRemaining = first.BurstRemaining - quantum;
                        availableCPUs.Insert(availableCPUs.Count, first);
                        availableCPUs.RemoveAt(0);
                    }
                    ...
                    if (availableCPUs.Count > 0)
                    {
                        foreach (var p in availableCPUs)
                        {
                            ...
                            processData[p.ProcessIndex, 2] += timeSpent;
                        }
                    }
                }
                else
                ...
                {
                    EamonProcess first = availableIOs.First();
                    ioProcesses.Add(first);

                    // how much time we spent working on a process.
                    // This will either be thhe value of the quantum, or the value of the process duration < quantum.
                    int timeSpent;

                    if (first.BurstRemaining < quantum)
                    {
                        ioTime = timeSpent = first.BurstRemaining;
                        first.BurstRemaining = 0;
                        availableIOs.RemoveAt(0);
                    }
                    else
                    {
                        ioTime = timeSpent = quantum;
                        first.BurstRemaining = first.BurstRemaining - quantum;
                        availableIOs.Insert(availableIOs.Count, first);
                        availableIOs.RemoveAt(0);
                    }
